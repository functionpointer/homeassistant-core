...
@pytest.mark.parametrize("first_input, second_input, expected_result", [
    (
            {
                CONF_DEVICE: "mqtt",
                CONF_VERSION: "2.3",
...
async def test_duplicate(hass: HomeAssistantType, first_input: Dict, second_input: Dict, expected_result: Optional[Tuple[str, str]]):
    await setup.async_setup_component(hass, "persistent_notification", {})
