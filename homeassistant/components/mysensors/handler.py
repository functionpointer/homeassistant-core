...
async def handle_set(hass, hass_config: ConfigEntry, msg):
...
async def handle_internal(hass, hass_config: ConfigEntry, msg):
...
async def handle_battery_level(hass, hass_config: ConfigEntry, msg):
    """Handle an internal battery level message."""
    _handle_node_update(hass, msg)


@HANDLERS.register("I_HEARTBEAT_RESPONSE")
async def handle_heartbeat(hass, hass_config: ConfigEntry, msg):
    """Handle an heartbeat."""
    _handle_node_update(hass, msg)


@HANDLERS.register("I_SKETCH_NAME")
async def handle_sketch_name(hass, hass_config: ConfigEntry, msg):
    """Handle an internal sketch name message."""
    _handle_node_update(hass, msg)


@HANDLERS.register("I_SKETCH_VERSION")
async def handle_sketch_version(hass, hass_config: ConfigEntry, msg):
    """Handle an internal sketch version message."""
    _handle_node_update(hass, msg)


@HANDLERS.register("I_GATEWAY_READY")
async def handle_gateway_ready(hass, hass_config: ConfigEntry, msg):
...
@callback
def _handle_child_update(hass, hass_config: ConfigEntry, validated):
